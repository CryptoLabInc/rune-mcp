...
// Tests for policy.Parse — port of the original query-processor test suite
// plus thoroughness-extending cases for every QueryIntent and TimeScope, regex
// precedence (rule iteration order), clean/cap invariants exact-match, and
// stop-word filter.
//
// Multilingual tests intentionally omitted: Go ParsedQuery has no Language
// field (agent pre-translates before invocation), so the regex/LLM
// split that existed in the original query processor does not exist here.
//
...
// decision_rationale / feature_history / pattern_lookup / technical_context /
// general_query.
//
// SECURITY_COMPLIANCE / HISTORICAL_CONTEXT / ATTRIBUTION share the regex tables,
// so we gate them too.
...
// targets. Tightened from the looser original assertion (which used OR): both
...
// token in cleaned. Preserved here as a single test rather than split
// across files.
...
// filtered, dedup'd post-lowercase. Tightened from the original OR to AND —
// both content terms must surface.
...
// overflows — ParsedQuery (entities ≤ 10, keywords ≤ 15, expansions ≤ 5).
