...
//   - custom thresholds ({0.4, 0.7, 0.93}, which
...
// → /10000 = 0.03). What we DO avoid is the round-half-away-from-zero
// divergence at exact .x...5 boundaries.
...
// score rounding — Go math.Round rounds half away from zero.
//
...
func TestClassifyNovelty_ScoreRounding(t *testing.T) {
	cases := []struct {
		...
		// (5th decimal is 4, no half-boundary)
		...
		// **divergence witness** — sim=0.12355 makes
		// (1-sim)*10000 = 8764.5 EXACTLY. Go's math.Round picks
		// away-from-zero (8765 → 0.8765). We assert the Go value here.
		// If this case starts failing because Go gives 0.8764, that means
		// novelty.go switched to banker's — update the production-code TODO
		// above.
...
// runtime D11 default. The values used here are 0.4 / 0.7 / 0.93.
//
// The runtime D11 mismatch is part of the agent-delegated SOT.
...
// DefaultNoveltyThresholds — runtime D11 values.
