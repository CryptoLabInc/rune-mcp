...
// TestTopKLimitErr — a wrapped console top_k rejection is converted to a distinct
...
func TestTopKLimitErr(t *testing.T) {
	topk := &console.Error{Code: console.ErrConsoleTopKExceeded.Code, Message: "top_k 8 exceeds limit 3 for role 'researcher'"}
	...
	cases := []struct {
		...
		{"direct console topk error", topk, true},
		{"wrapped console topk error", fmt.Errorf("console decrypt scores: %w", topk), true},
		{"generic invalid input", &console.Error{Code: console.ErrConsoleInvalidInput.Code}, false},
		{"unavailable", &console.Error{Code: console.ErrConsoleUnavailable.Code}, false},
		{"non-console error", errors.New("boom"), false},
		{"nil", nil, false},
	}
