...
// Domain — decision subject-area enum (19 values).
type Domain string
...
// SourceRef — reference to the source a record was captured from.
...
// Evidence — a claim backed by a quote and its source.
...
// Assumption — an assumption made in reaching the decision, with a confidence.
type Assumption struct {
	Assumption string  `json:"assumption"`
	Confidence float64 `json:"confidence"` // default 0.5
}

// Risk — a risk of the decision, with an optional mitigation.
type Risk struct {
	Risk       string  `json:"risk"`
	Mitigation *string `json:"mitigation,omitempty"`
}

// DecisionDetail — what was decided, and by whom / where / when.
...
// Context — problem framing and options considered around the decision.
type Context struct {
...
// Why — rationale and certainty behind the decision.
type Why struct {
...
// Quality — scribe confidence and human review state for the record.
type Quality struct {
...
// DecisionRecord — main record schema (v2.1).
...
// MaxTitleLen — title truncation limit, UTF-8 rune-aware.
...
// GenerateRecordID builds a record ID from timestamp, domain, and title.
...
func GenerateRecordID(ts time.Time, d Domain, title string) string {
	...
	for _, w := range words {
		if isAlnum(w) || isAlnum(strings.ReplaceAll(w, "_", "")) {
			kept = append(kept, w)
		}
	}
...
// GenerateGroupID — same slug rule as GenerateRecordID, "grp_" prefix.
...
// isAlnum reports whether s is non-empty and every rune is a letter or digit:
//   - "" → false
...
// EnsureEvidenceCertaintyConsistency enforces the evidence/certainty invariant:
...
// ValidateEvidenceCertainty — read-only. Returns false if the invariant is violated.
